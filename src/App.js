...
import Routes from './router-nav/Routes.js';

export const TOKEN_STORAGE_ID = "trainly-token";
...
function App() {
   ...
   */
   async function signup(signupData) {
    try {
      let token = await TrainlyApi.signup(signupData);
      setToken(token);
      return { success: true };
    } catch (errors) {
  ...
  async function login(loginData) {
    try {
      let token = await TrainlyApi.login(loginData);
      setToken(token);
      return { success: true };
    } catch (errors) {
